...
export class StudentListComponent {
  ...
  displayedColumns = ['id','firstName','lastName','isActive','absence', 'notes','edit','delete'];
  ...
  constructor(private readonly dialogService: MatDialog){}
     ...
     dialog.afterClosed().subscribe((value)=> {
      if (value) {
       const lastId = this.students[this.students.length - 1]?.id;
        ...
        this.students = [...this.students, new Student(lastId + 1, value.firstName, value.lastName, true, value.absence, value.notes)];
      }
    })
    ...
    editStudent(student: Student){
      const dialog = this.dialogService.open(StudentModalComponent,{ data: student,})
